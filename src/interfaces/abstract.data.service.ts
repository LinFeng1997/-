...
export abstract class AbstractDataService implements iDataService {
  ...
  * @type {string}
  */
    static QueryTimeoutEvent: string = "QueryTimeoutEvent";

    /**
     * 认证失效
     * @type {string}
     */
    static AuthenticationFailureEvent: string = "AuthenticationFailureEvent";

    /**
     * 通用查询失败
     * @type {string}
     */
    static QueryErrorEvent: string = "QueryErrorEvent";

    //构造配置文件
    //
    constructor(protected cfg: AppConfig, protected event?: Events) {
        this.Config = cfg;
        AbstractDataService.Event = event;
    ...
    protected handleError(error: any): ErrorObservable {
        ...
        try {
            ...
            console.log(ex);
            if (errorDesc.message == 'Timeout has occurred'){
                errorDesc.message = '网络操作超时，请检查您的网络并稍后重试！';
            }
        ...
        //发布数据查询错误事件
        try {
            if (AbstractDataService.Event != null)
                AbstractDataService.Event.publish(AbstractDataService.QueryErrorEvent, error.message);
        }
        catch (ex) {
            console.warn(ex);
        }
