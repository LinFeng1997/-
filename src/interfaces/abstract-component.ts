...
import {
    ...
    PopoverController, Popover, ActionSheetController, NavParams, Events
} from "ionic-angular";
...
export class AbstractComponent {
    ...
    protected tabChangeHanlder = null;

    protected locationChangeHanlder = null;

    protected queryErrorHanlder = null;

    protected queryTimeOutHanlder = null;
   ...
   */
    ...
    subscribeTimeout() {
        if(AppConfig.debug)
        console.info(this.eventCtrl, "订阅超时事件"); 
        //注册默认事件
        if (this.eventCtrl != null) {

            this.queryErrorHanlder = (e) => {
                this.catchDataServiceError(this, e);
            };

            this.queryTimeOutHanlder = (e) => {
                this.catchDataServiceTimeoutError(this, e);
            };

            //数据查询，通用处理程序
            this.eventCtrl.subscribe(AbstractDataService.QueryErrorEvent, this.queryErrorHanlder);
            this.eventCtrl.subscribe(AbstractDataService.QueryTimeoutEvent, this.queryTimeOutHanlder);
        }
    }

    unsubscribeTimeout() {
        if(AppConfig.debug)        
        console.info(this.eventCtrl, "取消订阅超时事件");                
        //  this.ngOnDestroy();//dyong
        //注销数据处理程序
        if (this.eventCtrl != null) {

            //
            if (this.queryErrorHanlder != null) {
                this.eventCtrl.unsubscribe(AbstractDataService.QueryErrorEvent, this.queryErrorHanlder);
                this.eventCtrl.unsubscribe(AbstractDataService.QueryTimeoutEvent, this.queryTimeOutHanlder);
            }

            //
            if (this.locationChangeHanlder != null) {
                // this.eventCtrl.unsubscribe(GpsInfoService.LocationChangedEvent, this.locationChangeHanlder);
            }
     ...
     * @param opts    其他参数
     */
    showModal(content: any, params?: any, opts?: any): Modal {
   ...
   */
    ...
    openExternalUrl(title: string, url: string): void {
        console.log(this.themeableBrowser);
        const options: ThemeableBrowserOptions = {
            ...
            backButton: {
                image: './assets/www/assets/images/logo.png',
    ...
    catchDataServiceError(sender, e) {
        if(AppConfig.debug) console.info("网络出错了朋友");
        //关闭任何现有的载入框
        if (sender != null) {
            sender.closeLoading();

            if (e != null && sender.toastCtrl != null) {
                sender.showMessage(e);
            }
        }
    }

    catchDataServiceTimeoutError(sender, e) {
        if(AppConfig.debug) console.info('网络操作超时，请检查您的网络状态并稍后重试！');
        if (sender != null && sender.toastCtrl != null) {
            sender.showMessage('网络操作超时，请检查您的网络状态并稍后重试！');
        }
    }
