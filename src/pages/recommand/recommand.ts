...
export class RecommandPage extends AbstractComponent implements OnInit {
	...
	addToCache(value) {
		...
		// 键值对系列
		let arr = [];
		this.cacheService.addCache(this.cfg.cacheKeys.username,
			{
				user: value
			});

		// this.cacheService.addCache(this.cfg.cacheKeys.username,value);
	}

	getCache() {
		var cacheKey = this.cfg.cacheKeys.username;
		try {
