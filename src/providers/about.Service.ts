...
export class AboutService extends AbstractService {
  ...
  */
    ...
    queryComment(classId): Observable<any> {
        //交换临时url和基础url
        [this.cfg.config.webBaseUrl, this.cfg.config.webTmpUrl] = [this.cfg.config.webTmpUrl, this.cfg.config.webBaseUrl];
        let res = this.dataSvc.getData(`comment/${classId}`, null);
        //交换回去临时url和基础url
        [this.cfg.config.webBaseUrl, this.cfg.config.webTmpUrl] = [this.cfg.config.webTmpUrl, this.cfg.config.webBaseUrl];
        return res;
    }

    postComment(cBody,cClass):Observable<any>{
        //交换临时url和基础url
        [this.cfg.config.webBaseUrl, this.cfg.config.webTmpUrl] = [this.cfg.config.webTmpUrl, this.cfg.config.webBaseUrl];
        let res = this.dataSvc.postData(`comment/${cClass}`, {comment:cBody});
        //交换回去临时url和基础url
        [this.cfg.config.webBaseUrl, this.cfg.config.webTmpUrl] = [this.cfg.config.webTmpUrl, this.cfg.config.webBaseUrl];
        return res;
    }

    getNewGrade():Observable<any>{
        //交换临时url和基础url
        [this.cfg.config.webBaseUrl, this.cfg.config.webTmpUrl] = [this.cfg.config.webTmpUrl, this.cfg.config.webBaseUrl];
        let res = this.dataSvc.postData(`newGrade`, null);
        //交换回去临时url和基础url
        [this.cfg.config.webBaseUrl, this.cfg.config.webTmpUrl] = [this.cfg.config.webTmpUrl, this.cfg.config.webBaseUrl];
        return res;
    }
